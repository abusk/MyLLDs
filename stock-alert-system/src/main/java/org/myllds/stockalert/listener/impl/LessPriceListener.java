...
public class LessPriceListener implements StockListener {
    private final Map<String, Integer> companyWithThreshold;

    public LessPriceListener(Map<String, Integer> companyWithThreshold) {
    ...
    public void onStockDataReceived(StockData stockData) {
        ...
        if (stockData.getPrice() < threshold) {
            System.out.println("Alert: " + stockData.getCompany() + " price is less than " + threshold);
        }
    }
}
